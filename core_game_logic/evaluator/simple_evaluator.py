...
class HandResult:
    ...
    """
    rank: HandRank              # 牌型等级
    primary_value: int          # 主要牌值（如对子的点数）
    secondary_value: int = 0    # 次要牌值（如两对中较小的对子）
    kickers: Tuple[int, ...] = ()  # 踢脚牌（按大小降序排列）
    ...
    def __str__(self) -> str:
        ...
        elif self.rank in [HandRank.THREE_KIND, HandRank.FOUR_KIND]:
            return f"{rank_name}({Rank(self.primary_value).name})"
        elif self.rank == HandRank.FULL_HOUSE:
            return f"{rank_name}({Rank(self.primary_value).name}带{Rank(self.secondary_value).name})"
        elif self.rank in [HandRank.STRAIGHT, HandRank.STRAIGHT_FLUSH]:
...
class SimpleEvaluator:
    ...
    def _evaluate_five_cards(self, cards: List[Card]) -> HandResult:
        ...
        if count_groups[4] == 1:  # 四条
            ...
            return HandResult(HandRank.FOUR_KIND, four_rank, kickers=(kicker,))
        ...
        if count_groups[3] == 1:  # 三条
            ...
            return HandResult(HandRank.THREE_KIND, three_rank, kickers=kickers)
