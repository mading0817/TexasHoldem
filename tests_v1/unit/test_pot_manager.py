...
class TestPotManager:
    ...
    def test_simple_pot_collection(self):
        ...
        # 使用合法的下注方法而不是直接设置current_bet
        self.players[0].bet(10)
        self.players[1].bet(10)
    ...
    def test_three_way_all_in_side_pots(self):
        ...
        # 使用合法的下注方法设置不同的All-in金额：A=25, B=50, C=100
        self.players[0].bet(25)  # 玩家A全押25
        self.players[1].bet(50)  # 玩家B全押50
        self.players[2].bet(100) # 玩家C下注100
    ...
    def test_pot_award_single_winner(self):
        ...
        assert self.players[0].chips == 140  # 原100 - 20(下注) + 60(奖励) = 140
        assert self.pot_manager.main_pot == 0  # 主池已清空
        
        print("[OK] 单一获胜者底池分配测试通过")
    ...
    def test_pot_award_split_pot(self):
        ...
        # 使用合法的下注方法设置底池
        self.players[0].bet(15)
        self.players[1].bet(15)
        self.players[2].bet(15)
        self.pot_manager.collect_from_players(self.players)
        ...
        # 最终筹码：原100 - 15(下注) + 奖励
        assert self.players[0].chips == 108  # 100 - 15 + 23 = 108
        assert self.players[1].chips == 107  # 100 - 15 + 22 = 107
    ...
    def test_complex_side_pot_award(self):
        ...
        # 设置复杂的All-in场景 - 使用合法的下注流程
        # A全押30
        actual_bet_a = self.players[0].bet(30)
        assert actual_bet_a == 30
        
        # B全押60
        actual_bet_b = self.players[1].bet(60)
        assert actual_bet_b == 60
        
        # C下注90
        actual_bet_c = self.players[2].bet(90)
        assert actual_bet_c == 90
...
def run_tests():
    ...
    try:
        ...
        print("=" * 50)
        print("[PASS] 所有测试通过！PotManager边池系统工作正常。")
        return True
