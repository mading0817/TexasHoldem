...
class TestPhaseTransition:
    ...
    def test_phase_transition_rollback_on_invalid_state(self):
        ...
        try:
            with phase_transition(self.state):
                # 创建无效状态：FLOP阶段但没有公共牌 - 测试环境允许直接设置
                self.state.phase = GamePhase.FLOP
                # 故意不设置公共牌，导致状态无效
                pass
        except (GameStateError, PhaseTransitionError):
            pass  # 预期的异常
...
def run_tests():
    ...
    try:
        ...
        print("=" * 50)
        print("[PASS] 所有测试通过！Phase转换机制工作正常。")
