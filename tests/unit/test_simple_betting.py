...
def test_simple_betting():
    ...
    if total_accounted == total_before:
        print("[OK] 总额匹配（底池 + 返还 = 原始投入）")
    else:
        print(f"[ERROR] 总额不匹配: 期望{total_before}, 实际{total_accounted}")
        
    # 验证玩家下注已重置
    for i, player in enumerate(players):
        if player.current_bet == 0:
            print(f"[OK] 玩家{i}下注已重置")
        else:
            print(f"[ERROR] 玩家{i}下注未重置: {player.current_bet}")
            
    # 验证返还逻辑
    if returns:
        for seat_id, amount in returns.items():
            print(f"[OK] 玩家{seat_id}获得返还{amount}筹码")
    else:
        print("[OK] 无需返还筹码")
