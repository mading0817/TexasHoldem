...
class TestGameState:
    ...
    def test_betting_round_control(self):
        ...
        assert not self.state.is_betting_round_complete(), "下注不等时下注轮应该未完成"
        
        # 测试下注轮完成的情况
        self.players[1].bet(5)  # 补齐到10
        self.players[2].bet(10)
        self.state.street_index = 3  # 所有人都行动过
